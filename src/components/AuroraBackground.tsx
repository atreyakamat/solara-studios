  ...
  return (
    <div className={`relative min-h-screen overflow-hidden bg-[#131118] ${className}`}>
      ...
      <div className="absolute inset-0 pointer-events-none">
        {/* Purple beam */}
        <motion.div
          className={`absolute left-1/4 top-0 h-full w-16 bg-gradient-to-b from-[#a89db9]/70 via-[#6211d4]/60 to-transparent opacity-70 ${isMobile ? 'blur-lg' : 'blur-2xl'} rotate-[-24deg]`}
          animate={isMobile ? undefined : { x: [0, 60, 0], y: [0, 120, 0], opacity: [0.7, 0.9, 0.7] }}
          transition={isMobile ? undefined : { duration: 10, repeat: Infinity, ease: "easeInOut" }}
        />
        {/* 45-degree teal/cyan beam */}
        <motion.div
          className={`absolute left-[50%] top-0 h-full w-14 bg-gradient-to-b from-[#00ffd0]/60 via-[#00bfff]/40 to-transparent opacity-60 ${isMobile ? 'blur-lg' : 'blur-2xl'} rotate-[45deg]`}
          animate={isMobile ? undefined : { x: [0, 40, 0], y: [0, 100, 0], opacity: [0.6, 0.8, 0.6] }}
          transition={isMobile ? undefined : { duration: 15, repeat: Infinity, ease: "easeInOut" }}
        />
        {/* Blue beam (hide on mobile) */}
        {!isMobile && (
          <motion.div
            ...
            animate={{ x: [0, -80, 0], y: [0, 160, 0], opacity: [0.6, 0.8, 0.6] }}
            transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}
          />
        )}
        {/* Greenish beam (hide on mobile) */}
        {!isMobile && (
          <motion.div
            ...
            animate={{ x: [0, 40, 0], y: [0, 100, 0], opacity: [0.5, 0.7, 0.5] }}
            transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
          />
        )}
        {/* Pink beam (hide on mobile) */}
        {!isMobile && (
          <motion.div
            ...
            animate={{ x: [0, -50, 0], y: [0, 140, 0], opacity: [0.6, 0.8, 0.6] }}
            transition={{ duration: 16, repeat: Infinity, ease: "easeInOut" }}
          />
        )}
        {/* White beam (hide on mobile) */}
        {!isMobile && (
          <motion.div
            ...
            animate={{ x: [0, 30, 0], y: [0, 180, 0], opacity: [0.4, 0.6, 0.4] }}
